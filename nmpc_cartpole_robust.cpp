...
constexpr double kInitXd    = 0.005;
constexpr double kInitPhi   = 0.003;
constexpr double kInitPhid  = 0.001;

constexpr bool   kEnableFallbackPD = true;   // CasADi 失败时启用后备控制
constexpr double kSimHz            = 60.0;   // 画面刷新对应的目标仿真推进频率
// ===========================================================
...
bool IsFiniteState(const std::array<double, 4>& x) {
  for (double v : x) {
...
double ClampControl(double u) {
  if (!IsFinite(u)) return 0.0;
  return std::clamp(u, -1.0, 1.0);
}

// 一个简单的兜底控制器：不能保证性能最好，但能避免 CasADi 失败后直接清零。
double FallbackPD(const std::array<double, 4>& x) {
  // x = [cart_pos, cart_vel, pole_angle, pole_ang_vel]
  // 这里按你的模型定义，phi=0 附近是期望平衡点。
  constexpr double kp_x   = -0.40;
  constexpr double kd_x   = -0.35;
  constexpr double kp_phi = -8.00;
  constexpr double kd_phi = -1.20;

  double u = kp_x * x[0] + kd_x * x[1] + kp_phi * x[2] + kd_phi * x[3];
...
struct CasadiMpcWrapper {
  CasadiMpcWrapper() {
    ...
    casadi_int sz_iw = 0;
    casadi_int sz_w = 0;
    if (M_work(&sz_arg, &sz_res, &sz_iw, &sz_w) != 0) {
      throw std::runtime_error("M_work() 失败，无法获取 CasADi 工作区大小");
    }

    arg_.resize(static_cast<size_t>(sz_arg), nullptr);
    res_.resize(static_cast<size_t>(sz_res), nullptr);
    iw_.resize(static_cast<size_t>(sz_iw), 0);
    w_.resize(static_cast<size_t>(sz_w), 0.0);

    if (arg_.empty() || res_.empty()) {
      throw std::runtime_error("CasADi 输入/输出工作区大小异常");
    }
  ...
  double Solve(const std::array<double, 4>& x) {
    if (!IsFiniteState(x)) {
      throw std::runtime_error("输入状态存在 NaN/Inf");
    }
    ...
    // 某些生成函数对 memory 初始化更稳妥。
    int init_flag = M_init_mem(mem);
    if (init_flag != 0) {
      M_release(mem);
      throw std::runtime_error("CasADi M_init_mem() 失败");
    }
    ...
    if (!IsFinite(u_[0])) {
      throw std::runtime_error("CasADi 输出 u 不是有限数");
    }

    return static_cast<double>(u_[0]);
  }
...
void ComputeAndSetControl(const mjModel* m, mjData* d) {
  ...
  try {
    ...
    // XML 里 actuator 有 gear='50' 且 ctrlrange='-1 1'
    // MATLAB 模型里 actual_force = 50*u
    // 所以这里直接写 u 到 d->ctrl，不再乘 50。
    d->ctrl[g_act_id] = u;
    g_last_u = u;
    g_fail_count = 0;
    g_last_mode = "MPC";
    ...
    if (kEnableFallbackPD && IsFiniteState(x)) {
      u = FallbackPD(x);
      mode = "PD fallback";
    }
    ...
    // 限频打印，避免终端刷屏导致程序看起来卡顿
    if (g_fail_count <= 5 || g_fail_count % 50 == 0) {
      std::cerr << "[WARN] 控制器求值失败，第 " << g_fail_count
                << " 次, t=" << d->time
                << ", mode=" << g_last_mode
                << ", err=" << g_last_error << std::endl;
    }
  }
}
  ...
  try {
    ...
    while (!glfwWindowShouldClose(g_window)) {
      if (!g_paused) {
        ...
        while (g_data->time - simstart < 1.0 / kSimHz) {
